...
from transformers import AutoConfig, AutoModelForSequenceClassification, AutoTokenizer, T5ForConditionalGeneration
...
def parse_args():
    ...
    parser.add_argument('--llm',  action="store_true", help = "toggle to indicate use of generative model for prediction")

    parser.add_argument('--delta_weight', default=0.55, type=float, help='delta*delta_weight+embedding_init')
    parser.add_argument('--delta0_index', default=0, type=int)
    parser.add_argument('--delta1_index', default=0, type=int)
    parser.add_argument('--delta2_index', default=0, type=int)
    parser.add_argument('--delta3_index', default=0, type=int)
    ...
    parser.add_argument('--adv_file_path', type=str, default='none')
    parser.add_argument('--dev_file_path', type=str, default='none')
    parser.add_argument('--test_file_path', type=str, default='none')
    parser.add_argument('--detect_result_path', type=str, default='none')
    ...
    if args.dataset_name == 'sst2' or args.dataset_name == 'imdb' or args.dataset_name == 'rotten_tomatoes':
        args.num_labels = 2
    ...
    if(args.detect_result_path == 'none'):
...
def single_detect(
    ...
    if(args.dataset_name == 'glue'):
        args.tmp_dataset_name = args.dataset_name
        args.dataset_name = args.task_name # added to align with saved results in uap_HF
